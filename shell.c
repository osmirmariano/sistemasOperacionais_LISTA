...
#define MAX 80 /*Definindo o tamanho máximo do comando*/
#define TAM 50
...
/*-----------------------------------FUNÇÃO PRINCIPAL----------------------------------*/
int main(){
    char armazenaEntrada[MAX]; /*Para armazenar a entrada do comando*/
    char *args[MAX/2+1]; /*Definindo a quantidade de argumentos máximos */
    int should_run = 1, flag, i; 
    pid_t pid, tpid;

    printf("\n\nPara visualizar o histórico do terminal, digite 'historico'\n\n");
...
int comandosShell(char texto[], char *args[], int *flag){
    ...
    tamanho = read(STDIN_FILENO, texto, MAX);//tamanho recebe a quantidade de dados lido pela função read
    inicio = -1;
